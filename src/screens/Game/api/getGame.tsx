...
async function getGame(gameId: Games["id"]) {
  const query = `fields name, cover.url, summary, genres.name, involved_companies.developer, involved_companies.porting, involved_companies.publisher, involved_companies.supporting, involved_companies.company.name, videos.name, videos.video_id, release_dates.*, release_dates.platform.abbreviation, release_dates.platform.name;
...
export const useGame = (gameId?: Games["id"]) =>
  useQuery({
    ...
    queryFn: () => getGame(gameId as Games["id"]),
    enabled: typeof gameId === "number" && Number.isFinite(gameId),
