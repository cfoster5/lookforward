...
export async function getUpcomingGameReleases(igdbToken: string): Promise<IGDB.ReleaseDate.ReleaseDate[]> {
...
export async function searchGames(searchVal: string, igdbToken: string): Promise<IGDB.Game.Game[]> {
  const response = await fetch("https://api.igdb.com/v4/games", {
    method: 'POST',
    headers: {
      "Client-ID": "lj3tlp1tz4vmha6gousdpge7x12s5m",
...
export async function getIGDBToken() {
  return await firestore().collection('igdbAuth').doc("creds").get();
}
